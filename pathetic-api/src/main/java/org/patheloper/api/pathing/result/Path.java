...
public interface Path extends Iterable<PathPosition> {
   ...
   * the original path such that no two consecutive positions are more than `resolution` blocks apart. The interpolated
   * positions are computed using linear interpolation, which means that the resulting path will have a smooth curve
   * that passes through each of the original positions.
   *
   * @param resolution The desired distance between consecutive positions in the resulting path, in blocks. A lower
   *                   value will result in a higher resolution and a smoother curve, but will also increase the number
   *                   of positions in the resulting path and possibly reduce performance.
   *
  ...
  /**
   * Simplifies the path by removing intermediate positions based on the given epsilon value. The start and end
   * positions are always included in the simplified path.
   *
   * @param epsilon the epsilon value representing the fraction of positions to keep (should be in the range greater
   *                than 0.0 to 1.0, inclusive)
   *
