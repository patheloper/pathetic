 ...
 * parameters, you can fine-tune the pathfinding process to suit the specific needs of your Minecraft environment.
 */
@With
@Value
@Getter
...
public class PathfinderConfiguration {
  ...
  /**
   * The maximum permissible length of a calculated path (in blocks). Use this to constrain long searches that could
   * impact performance. A value of 0 indicates no limit.
   */
  int maxLength;
  ...
  /**
   * Controls whether the pathfinding algorithm can take diagonal steps. Enabling this allows for more flexible and
   * potentially shorter paths but might require a slightly more refined heuristic.
   *
   * @default true
   */
  @Builder.Default
  ...
  /**
   * If set to true, the pathfinding process will terminate immediately if no path is found between the start and
   * target. This can be helpful for quick validation but prevents fallback strategies.
   */
   ...
   * calculated path. This can help maintain progress, but might use an outdated path.
   */
  boolean allowingFallback;

  /**
   * Controls whether chunks should be loaded or generated as needed during the pathfinding process. This is essential
   * for exploring uncharted areas, but may impact performance.
   */
  boolean loadingChunks;

  /**
   * If pathfinding fails, determines whether to run a reverse pathfinding check (from target to start) to verify the
   * result. This is a computationally expensive fallback but can help identify some failure cases.
   */
   ...
   */
  @Builder.Default
  HeuristicWeights heuristicWeights = HeuristicWeights.NATURAL_PATH_WEIGHTS;
